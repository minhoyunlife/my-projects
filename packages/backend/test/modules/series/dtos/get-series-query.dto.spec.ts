...
describeWithoutDeps('GetSeriesQueryDto', () => {
  const validDtoData: Partial<GetSeriesQueryDto> = {
    page: 1,
    search: 'test',
  };
  ...
  describe('page', () => {
    it('값이 유효한 경우, 에러가 발생하지 않음', async () => {
      const dto = createDto(GetSeriesQueryDto, validDtoData);
    ...
    it('값이 소수인 경우, 에러가 발생함', async () => {
      const dto = createDto(GetSeriesQueryDto, validDtoData, { page: 1.23 });
      const errors = await validate(dto);
    ...
    it('값이 1 미만인 경우, 에러가 발생함', async () => {
      const dto = createDto(GetSeriesQueryDto, validDtoData, { page: 0 });
      const errors = await validate(dto);
  ...
  describe('search', () => {
    ...
    it('빈 값이 주어진 경우, 에러가 발생함', async () => {
      const dto = createDto(GetSeriesQueryDto, validDtoData, { search: '' });
      const errors = await validate(dto);
